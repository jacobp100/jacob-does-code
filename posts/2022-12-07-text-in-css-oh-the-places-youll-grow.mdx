...
export const title = "Text in CSS: Oh, the Places You'll Grow";
export const css = ["/assets/blog.css", "/assets/blog-highlight.css"];

# Text in CSS: Oh, the Places You'll Grow
...
This technique works more broadly than buttons: it's great anywhere where you're applying padding around some text &mdash; like text inputs or dropdowns. It's how iOS sizes the rows in its table view component.
...
However, when looking at OSes across different form factors, font sizes don't change all that much. For example, the font size on an iPhone is identical to the font size on an iPad &mdash; and the font size on a MacBook is smaller than both.
